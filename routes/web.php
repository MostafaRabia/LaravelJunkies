...
Route::post('edit/product/{id}','HomeController@edit_product_post')->where('id','[0-9]+');
Route::post('edit/{id}','HomeController@edituser_post')->where('id','[0-9]+');
Route::post('edit/user/{id}','Users@editUserPost')->where('id','[0-9]+');
Route::post('/checkout',[
...
Route::group(['middleware'=>'admin'],function(){
	...
	Route::get('edit/product/{id}','HomeController@edit_product')->where('id','[0-9]+');
	Route::get('profiles-list','HomeController@profiles_list');
	Route::get('delete/user/{id}','HomeController@deleteuser')->where('id','[0-9]+');
	Route::get('edit/{id}','HomeController@edituser')->where('id','[0-9]+');
});
/* End Admin */
...
Route::group(['middleware'=>'user'],function(){
	...
	Route::get('edit/user/{id}','Users@showEdit')->where('id','[0-9]+');
});
